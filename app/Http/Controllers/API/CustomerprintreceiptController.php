...
class CustomerprintreceiptController extends Controller
{
...
///////////////////////////////////////////////////////////////////////
    ...
    public function update(Request $request, $id)
    {
      ...
      $numbero = \DB::table('customersreporttoviews')->where('ucret', '=', $userid)->count();
if($numbero > 0)
{   
    $customertype = DB::table('customers')->where('id', $id)->value('customertype');
    /// getting the receipt no 
    $receiptno = \DB::table('customerpayments')->where('customername', '=', $ids)->orderBy('id', 'Desc')->value('receiptno');
       $result = DB::table('customersreporttoviews')->where('ucret', $userid)->update(
           [
               'customername' => $ids,
               'receiptno' => $receiptno,
...
{   
    ...
    $customertype = DB::table('customers')->where('id', $id)->value('customertype');
/// getting the receipt no 
$receiptno = \DB::table('customerpayments')->where('customername', '=', $cnamme)->orderBy('id', 'Desc')->value('receiptno');
    Customersreporttoview::Create([
