...
class ExpensesrecordstoviewtrioController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        /// checking if there exists a record
         $numbero = \DB::table('expenserecordtoselects')->where('ucret', '=', $userid)->count();
...
// $result2 = \DB::table('expenserecordtoselects')->where('ucret', $userid)->update(
...
$expensename = $request['expensename'];
$displaynumber = $request['displaynumber'];
$branch2 = strlen($branch);
$expensename2 = strlen($expensename);
$displaynumber2 = strlen($displaynumber);
if($branch2 > 0)
{
    DB::table('expenserecordtoselects')
            ->where('ucret', $userid)
            ->update(array(
                'branch' => $request['branchname']
              //  'expensename' =>  $request['expensename']
...
/// creating a new record for Non Existance
  ...
  if($numbero < 1){
       return Expenserecordtoselect::Create([
