...
{ 
  ...
  //  'branch', 'ucret','startdate','enddate',
    ...
    public function store(Request $request)
    {
        ...
        $existanceinthetable = \DB::table('expensereporttoviews')->where('ucret', '=', $userid)->count();
        if($existanceinthetable < 1 )
        { Expensereporttoview::Create([
          //  'branch', 'ucret','startdate','enddate',
            //    'productcode' => $request['productcode'],
                'startdate' => $request['startdate'],
                'enddate' => $request['enddate'],
...
{ 
    ...
    {
         DB::table('expensereporttoviews')
 ->where('ucret', $userid)
 ->update(['wallet' => $request['wallet']
