...
$productcode  = \DB::table('products')->orderBy('id', 'Desc')->limit(1)->value('id');
  ...
  //    'productcode' => $request['productcode'],
    ...
    public function salesreturnsrecords(){

       // return   Salesreturn::with(['brandName','productCategory','productSupplier','unitMeasure'])->orderBy('id', 'Asc')
       return   Salesreturn::with(['productName','branchName','cashierName', 'unitName','unitNamereturned','userRecieving','userCreating'])->orderBy('id', 'Asc')
      //  return   Salesreturn::orderBy('id', 'Asc')
        //  ->where('category', $productcategory)
          //  ->where('brand', $productbrand)
          //  ->where('del', 0)
                ->paginate(20);
        // if($search = \Request::get('q')){
        //   $users = Salesreturn::where(function($query) use ($search){
      ...
      public function purchasereturnsrecords(){
        if($search = \Request::get('q')){
          $users = Purchasereturn::where(function($query) use ($search){
            $query->where('productname', 'LIKE', "%$search%");
          //  ->where('uracode', 'LIKE', "%$search%");
          })
            -> paginate(30);
