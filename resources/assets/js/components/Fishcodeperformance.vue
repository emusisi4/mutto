...
<!--  -->
  ...
  <label> Branch :</label>
                            <select v-model="branch" id="xx3"    @change='getFloatcodes()'>
                              ...
                              <option v-for='data in branches' :value='data.id'>{{ data.branchname }}</option>
                            </select>
        ...
        <label for="exampleInputEmail1">Float code :</label>
         ...
         <select name ="codeinuse" v-model="form.codeinuse" id ="branch" v-on:change="myClickEventtosavesalesreportbydate"   :class="{'is-invalid': form.errors.has('codeinuse')}">
...
<option  v-for='data in floatcodes' v-bind:value='data.floatcode'> {{ data.floatcode }}</option>
       ...
       <div class="bethapa-reportheader-header" > FLOAT CODE PERFPRMANCE 
 ...
 <div class="progress" style="height: 25px; font-size: 19px;">
    ...
    <div class="card-footer">
                <ul class="pagination pagination-sm m-0 float-right">
                   <pagination :data="codedetails" @pagination-change-page="paginationResultcodedetails"></pagination>
                </ul>
              </div>
...
getFloatcodes: function() {
  ...
  paginationResultdailyreport(page = 1) {
                        axios.get('api/codedetails?page=' + page)
                          .then(response => {
...
paginationResultcodedetails(page = 1) {
                        axios.get('api/codedetails?page=' + page)
                          .then(response => {
                            this.codedetails = response.data;
                          });
