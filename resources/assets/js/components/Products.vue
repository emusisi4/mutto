...
<label>Records Per page :</label>
                    <select name ="displaynumber" v-model="form.displaynumber" id ="displaynumber" class="fshow-tick"   v-on:change="tosubmitProductcategoryfilter"  :class="{'is-invalid': form.errors.has('expensename')}">
...
<!--  -->
                                ...
                                <td>{{prodcates.id}}</td>
                                 <td>{{prodcates.catname | firstletterCapital }}</td>
                                 ...
                                 <td>{{prodcates.created_at | myDate2 }}</td>  
                                 <td> 
                                  
                              <button type="button" v-if="allowedtoeditbranch > 0 "   class="btn  bg-gradient-secondary btn-xs fas fa-edit"  @click="editProductcategory(prodcates)">Edit Record</button>
                             


                            <button type="button"  class="btn  bg-gradient-danger btn-xs fas fa-trash-alt" @click="deleteProductCategory(prodcates.id)"> Delete Record </button>
...
<!--  -->
                          <tr v-for="probrands in productbrandsrecords.data" :key="probrands.id">
                                ...
                                <td>{{probrands.id}}</td>
                                 <td>{{probrands.brandname | firstletterCapital }}</td>
                              ...
                              <button type="button" v-if="allowedtoeditbranch > 0 "   class="btn  bg-gradient-secondary btn-xs fas fa-edit"  @click="editProductbrand(probrands)">Edit Record</button>
                             


                            <button type="button"  class="btn  bg-gradient-danger btn-xs fas fa-trash-alt" @click="deleteProductbrand(probrands.id)"> Delete Record </button>
       ...
       <tr>
            ...
            <th colspan="1"  style="font-size: 18px; text-align:center;    
             ...
             <th colspan="2"  style="font-size: 18px;   text-align:center;  border-bottom: 4px solid rgb(124 102 102); 
                ...
                <th colspan="1"  style="font-size: 18px; text-align:center;    
               border-bottom: 4px solid rgb(124 102 102);     background-color: rgb(29 31 34 / 37%); color: #131378;"> COST PRICE</th>
  
  
               <th colspan="2"  style="font-size: 18px; text-align:center;    
               border-bottom: 4px solid rgb(124 102 102);     background-color: rgb(29 31 34 / 37%); color: #131378;"> SELLING PRICES</th>

   <th colspan="2"  style="font-size: 18px; text-align:center;    
               border-bottom: 4px solid rgb(124 102 102);     background-color: rgb(29 31 34 / 37%); color: #131378;"> DISCOUNT PRICE</th>
          ...
          <tr v-for="prodcates in productdetailsrecords.data" :key="prodcates.id">
            ...
            <td>  <template v-if="prodcates.unit_measure">	{{prodcates.unit_measure.shotcode}}</template> -
              <template v-if="prodcates.unit_measure">	{{prodcates.unit_measure.unitname}}</template></td>  
                ...
                <td><div class="musisialignright">  {{formatPrice(prodcates.creditsellingprice)}} </div></td>
  ...
  <div class="switch"  v-if="prodcates.discountstatus == '0' ">
                ...
                <td><div class="musisialignright">  {{formatPrice(prodcates.discountedprice)}} </div></td>
                 ...
                 <td><div class="musisialignright">  {{formatPrice(prodcates.unitprice - prodcates.unitcost)}} </div></td>
                  <td>
                    <div v-if="prodcates.creditsellingprice > 0 " class="musisialignright">  {{formatPrice(prodcates.creditsellingprice - prodcates.unitcost)}} </div>
                     <div v-if="prodcates.creditsellingprice < 1 " class="musisialignright">  0 </div>  
                    </td>
...
<div class="modal fade" id="pruductdetailModal">
 ...
 <div v-show="editmode" class ="bethapa-table-sectionheader">Price Alteration Settings </div>   
    ...
    <div class="row clearfix" v-show="editmode">
                                    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-5 form-control-label">
                                        <label for="email_address_2">Credit Selling Price</label>
                                    </div>
                                    <div class="col-lg-10 col-md-10 col-sm-8 col-xs-7">
                                        <div class="form-group">
                                            <div class="form-line">
                                               <input v-model="form.creditsellingprice" type="number" name="creditsellingprice"
                      class="form-control" :class="{ 'is-invalid': form.errors.has('creditsellingprice') }">
                    <has-error :form="form" field="creditsellingprice"></has-error>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<div class="modal fade" id="paypurchaseinvoiceModal">
         <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h3  v-show="!editmode"    class="modal-title"><img src="images/logo.png" class="profile-user-img img-fluid img-circle" style="height: 80px; width: 80px;">ADD NEW UNIT</h3> 
                <h4  v-show="editmode" class="modal-title" ><img src="images/logo.png" class="profile-user-img img-fluid img-circle" style="height: 80px; width: 80px;">Make Invoice Payment </h4> 
                        </div>
...
// gettheinvoicedate
  ...
  saveproductcategoryFilter(){

                                this.loading = true;
                                this.form.post('api/productcategoryFiltertoview')
                                .then(()=>{
     axios.get("api/productcategoriesrecords").then(({ data }) => (this.productcategoriesrecords = data));
...
paginationResultsProductdetailsrecords(page = 1) {
   ...
   paginationResultsProductcategories(page = 1) {
                        axios.get('api/productcategoriesrecords?page=' + page)
                          .then(response => {
                            this.productcategoriesrecords = response.data;
                          });
                      },
  /// users
     laodIvoiceslist(){
...
$('#confirmitemonpurchase').modal('show');
   ...
   editProductcategory(productcategoriesrecords){
                this.editmode = true;
                 this.form.clear();
        this.form.reset();
...
/// send request ti
if (result.isConfirmed) {
  this.form.delete('api/productcategoriesrecords/'+id).then(()=>{
 ...
 axios.get("api/productcategoriesrecords").then(({ data }) => (this.productcategoriesrecords = data));
    ...
    createProductcategory(){
      this.$Progress.start();
        this.form.post('api/productcategoriesrecords')
        .then(()=>{
...
this.form.put('api/productcategoriesrecords/'+this.form.id)
  .then(()=> {
    // on success
   $('#addproductcategoryModal').modal('hide');
    Swal.fire(
