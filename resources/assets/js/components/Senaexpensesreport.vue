                        ...
                        <div class="body">
                            ...
                            <ul class="nav nav-tabs" role="tablist">
                                ...
                                <li role="presentation" class="active" v-if="categoriesComponentaccess > 0">
                                    <a href="#home_with_icon_title"  @click="loadDailyrecordreport()" data-toggle="tab">
                                        <i class="material-icons"></i> Daily Expenses Report Sumary
                                    </a>
                                </li>
                                ...
                                <li role="presentation">
                                    <a href="#profile_with_icon_title"  @click="loadSalesreportsummary()" data-toggle="tab">
                                        <i class="material-icons"></i> General Expenses Report - Filters
                                    </a>
                                </li>
...
Daily Expenses report summary : Rangingin from  {{salesreportsatartingdate}} to {{salesreportendingdate}}
</div>
...
<!-- <div class="mysalessect">  -->
 ...
 <form @submit.prevent="saveexpensereportdetailstoView()">
...
<table  class="musisireporttable" width="100%" border="1">
      ...
      <tr>
           ...
           <th colspan="1"  style="font-size: 18px; text-align:center;   
                border-bottom: 4px solid rgb(124 102 102); 
                  background-color: rgb(29 31 34 / 37%); color: #131378;"> AMOUNT</th>
...
<tr>
    <!-- axios.get("api/dailyexpensesreportsummary").then(({ data }) => (this.dailycodesreportdata = data)); -->
          <tr v-for="prodcates in dailyexpensesreportsummary.data" :key="prodcates.id">
              <td>{{prodcates.id }}</td>
...
<th style="font-size: 18px; text-align:center;    
               border-top: 4px solid rgb(124 102 102);    
                background-color: rgb(211 211 211); color: #131378;" >
<div class="musisialignright"> {{currencydetails}} {{formatPrice(totalexpensesinselectiond)}} </div>
</th>
                    ...
                    <button  v-show="!editmode" type="submit" class="btn btn-primary btn-sm">Create</button> 
                      <button v-show="editmode" type="submit" class="btn btn-success btn-sm" >Update</button>
                        <button  type="button" data-dismiss="modal" class="btn btn-danger btn-sm">Close</button >
                              ...
                              <div class="bethapa-reports-header">
            Expense Report Details : Rangingin from  {{salesreportsatartingdate}} to {{salesreportendingdate}}
...
totalexpensesinselectiond:{},
totalexpensesinselectionfilters:{},
...
updateunitsellingprice(event) {
 ...
 loadDailyrecordreport(){
        axios.get("api/dailyexpensesreportsummary").then(({ data }) => (this.dailyexpensesreportsummary = data));
      axios.get("api/totalexpensesinselectiond").then(({ data }) => (this.totalexpensesinselectiond = data));
...
// gettheinvoicedate
  ...
  saveexpensereportdetailstoView(){

                                this.loading = true;
                                this.form.post('api/saveexpensereportdetailstoView')
                                .then(()=>{
...
axios.get("api/salessummaryComponentaccess").then(({ data }) => (this.salessummaryComponentaccess = data));
  ...
  loadDailysalesreportdetails(){
      ...
      axios.get("api/totalexpensesinselectionfilters").then(({ data }) => (this.totalexpensesinselectionfilters = data));
    axios.get("api/totalexpensesinselectiond").then(({ data }) => (this.totalexpensesinselectiond = data));
        axios.get("api/totaldailytotalcostrangereports").then(({ data }) => (this.totaldailytotalcostrangereports = data));
        axios.get("api/totaldailygrossprofitrangereports").then(({ data }) => (this.totaldailygrossprofitrangereports = data));
          axios.get("api/totaldailytotalvatrangereports").then(({ data }) => (this.totaldailytotalvatrangereports = data));
