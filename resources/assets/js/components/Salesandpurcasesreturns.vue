                            ...
                            <ul class="nav nav-tabs" role="tablist">
                                ...
                                <li role="presentation" class="active" v-if="categoriesComponentaccess > 0">
                                    <a href="#home_with_icon_title" @click="loadProductcategories()" data-toggle="tab">
                                        <i class="material-icons"></i> Sales Returns (Returns In-Words)
                                    </a>
                                </li>
                                ...
                                <li role="presentation">
                                    <a href="#settings_with_icon_title" v-if="companyproductsComponentaccess > 0" @click="laodProductdetails()" data-toggle="tab">
                                        <i class="material-icons"></i> Purchase Returns (Returns Out-words)
                                    </a>
                                </li>
...
<table  class="musisireporttable" width="100%" border="1">
         ...
         <tr>
          ...
          <th colspan="1"  style="font-size: 18px; text-align:center;   
            ...
            <th colspan="7"  style="font-size: 18px; text-align:center;    
               border-bottom: 4px solid rgb(124 102 102);     background-color: rgb(29 31 34 / 37%); color: #131378;"> Returns </th>
     ...
     <tr>
                     ...
                     <th > Total </th>
                       ...
                       <th > Comment / Reason for return </th>
                   
</tr>
<!--  -->
                                <tr v-for="prodcates in salesreturnsrecords.data" :key="prodcates.id">
                                ...
                                <td>{{prodcates.id}}</td>
                                  ...
                                  <td> <template v-if="prodcates.branchname">	{{prodcates.branch_name.branchname}}</template> </td>
                                  <td> <template v-if="prodcates.cashiername">	{{prodcates.cashier_name.name}}</template> </td>
                                  <td> <template v-if="prodcates.productcode">	{{prodcates.product_name.productname}}</template> </td>
                                  <td> <template v-if="prodcates.unitofsalemeasure">	{{prodcates.unit_name.unitname}}</template> </td>
                                 <!-- unitNamereturned  -->
                                          <td>{{prodcates.qtysold}}</td>
                                          <td>{{prodcates.unitpricesold}}</td>
                                          
                                        <td>{{prodcates.totalamountsold}}</td>
   <td>{{prodcates.datereturned}}</td>
   <td> <template v-if="prodcates.userrecieving">	{{prodcates.user_recieving.name}}</template> </td>
   <td> <template v-if="prodcates.unitreturned">	{{prodcates.unit_namereturned.unitname}}</template> </td>
                                  <td>{{prodcates.qtyreturned}}</td>
                                  
                                   <td>{{prodcates.unitreturnprice}}</td>
                                  <td>{{prodcates.totalamountreturned}}</td>

                                  <td>{{prodcates.comment}}</td>
...
<div class="musisialignright">  </div>
    ...
    <!-- <div class="card-footer">
                <ul class="pagination pagination-sm m-0 float-right">
                   <pagination :data="salesreturnsrecords" @pagination-change-page="paginationResultsProductcategories"></pagination>
                </ul>
    </div> -->
...
// gettheinvoicedate
  ...
  saveproductcategoryFilter(){
                                ...
                                .then(()=>{
     axios.get("api/salesreturnsrecords").then(({ data }) => (this.salesreturnsrecords = data));
...
paginationResultsProductdetailsrecords(page = 1) {
   ...
   paginationResultsProductcategories(page = 1) {
                        axios.get('api/salesreturnsrecords?page=' + page)
                          .then(response => {
                            this.salesreturnsrecords = response.data;
                          });
                      },
  /// users
     laodIvoiceslist(){
...
$('#confirmitemonpurchase').modal('show');
   ...
   editProductcategory(salesreturnsrecords){
                this.editmode = true;
                 this.form.clear();
        this.form.reset();
        this.form.fill(salesreturnsrecords);
$('#addproductcategoryModal').modal('show');
            },
...
/// send request ti
if (result.isConfirmed) {
  this.form.delete('api/salesreturnsrecords/'+id).then(()=>{
 ...
 axios.get("api/salesreturnsrecords").then(({ data }) => (this.salesreturnsrecords = data));
    ...
    createProductcategory(){
      this.$Progress.start();
        this.form.post('api/salesreturnsrecords')
        .then(()=>{
...
this.form.put('api/salesreturnsrecords/'+this.form.id)
  .then(()=> {
    // on success
   $('#addproductcategoryModal').modal('hide');
    Swal.fire(
