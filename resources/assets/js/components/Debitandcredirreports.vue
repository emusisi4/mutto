                  ...
                  <li class="nav-item">
                    <a class="nav-link" id="custom-tabs-two-profile-tab"
                     data-toggle="pill" href="#custom-tabs-two-profile" role="tab"
                      @click="loadDailyrecordreport()"  aria-controls="custom-tabs-two-profile" aria-selected="false">Branch Collections && Credits Report</a>
...
<!--  -->
    ...
    <label for="exampleInputEmail1">Report type :</label>
         ...
         <select name ="reporttype" v-model="form.reporttype" id ="reporttype" :class="{'is-invalid': form.errors.has('reporttype')}">
<option value="">  </option>
<option value="1"> Collections Report </option>
<option value="2"> Credits Report </option>
...
<option v-for='data in orderlistfordatesalesreport' v-bind:value='data.sysname'> {{ data.sortname }}</option>
       ...
       <div class="bethapa-reportheader-header" > COLLECTIONS / CREDIT REPORT : <i> From {{ generalreportselectedstartdate|myDate2 }} To : {{ generalreportselectedenddate|myDate2 }}</i>
 ...
 <div class="col-lg-3 col-2" >
         ...
         <div class="small-box bg-pnne">
              <div class="inner">
                <h3>NET COLLECTIONS  </h3>

         
           <h5>   <b>{{currencydetails}}   {{formatPrice(totalrangecollections- totalrangecredits ) }}  
             (
              
               {{parseFloat((((totalrangecollections- totalrangecredits)/totalrangecollections))*100).toFixed(0)}}%
...
<table class="table">
    ...
    <tbody>
        <tr>
              <tr v-for="submenuinfo in collectiondetails.data" :key="submenuinfo.id">
...
dailycollection :null,
         ...
         genrealfishreportsAccess:'',
          currentlyselecteddebitcollectionsreport: '',
...
$('#addnewcomponentfeaturemodal').modal('show');
 ...
 Createrecordupdatebal(){
  ...
  title: 'Date Corrected successfully'
    ...
    $('#addnewcomponentfeaturemodal').modal('show');
 axios.get("api/collectiondetails").then(({ data }) => (this.collectiondetails = data));
  Toast.fire({
  icon: 'success',
  title: 'Date Corrected successfully'
});
...
this.loading = true;
                                this.$Progress.start();
                                this.form.post('api/dailycollectionsreport')
                                .then(()=>{
